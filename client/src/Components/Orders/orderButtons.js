...
function OrderButtons({id, userId, createdAt, precios, orders, putOrder, users, estado, putProduct, productsCar, products}) {
	...
	const handleStock = () => {
		products.map(p => {p.stock = p.stock + p.lineorder.cantidad  
		  console.log(p, p.id)               
		  putProduct(p, p.id)}
		)
	 }
